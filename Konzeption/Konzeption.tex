...
\image[FrameworkSeiten.pdf][width=0.7\linewidth]{BewertungFrameworkSeiten}{Bewertung der drei Frameworkseiten}{Die 3 Seiten eines Frameworks und dessen reguläre Bewertung}
...
Bei der Bewertung von Frameworks setzt \ac{MFEM} keine grundlegenden Kenntnisse über das Framework voraus, da es zwei Seiten der Analyse anbietet. Dies ist einerseits die Nutzung von bereits vorhandenem Expertenwissen, was die Phase der Evaluation verkürzt. Andererseits werden die restlichen Daten über die Erstellung von Prototypen erfasst.
...
\ac{MFEM} nutzt hierzu den aus \ac{ATAM} bekannten \acl{QUT}, um die passenden Anforderungen mittels Brainstorming zu finden. Doch bevor dies erfolgen kann, muss geklärt werden, welche Arten von Anforderungen für die Bewertung eines Frameworks von Interesse sind. Hierzu gibt es funktionale und nicht-funktionale Frameworkanforderungen.
...
eine Registrierungsstelle anbieten. Dort können sich sämtliche Service-Instanzen registrieren und auch andere Services finden. Ein Microservice muss eine Verbindung zur Servicediscovery aber auch nicht direkt unterstützen. Projekte wie Spring Cloud Sidecar\footnote{
\url{http://projects.spring.io/spring-cloud/spring-cloud.html\#_polyglot_support_with_sidecar}
}
übernehmen dies für einzelne Services.\\ 
Die Discovery kann aber auch auf die darüber liegende Abstraktions-Schicht hochgezogen werden. Innerhalb eines Kubernetes\footnote{
...
Somit kann die Umsetzung der Servicediscovery in jeder Architektur anders aussehen, wie in Abbildung \ref{Servicediscoverytypen} dargestellt. 
...
Um bei der Definition der Metriken stets das Ziel im Auge zu behalten, wird die bereits aus \ac{SAEM} bekannte \ac{GQM} Methode genutzt. Sie hat sich dort bewährt und bietet einen guten Top-Down-Ansatz. 

Eine um Metriken erweiterter Qualitätsbaum ist in Bild \ref{MFEMGQMQUTAusschnitt} dargestellt. Hierzu wurden 3 Anforderungen aus den Basisanforderungen als Beispiel verwendet.

\image[MFEMGQMQUTAusschnitt.pdf]{MFEMGQMQUTAusschnitt}{\acs{MFEM} Qualitätsanforderungen erweitert um Metriken}{Beispiele von Qualitätsanforderungen erweitert um Metriken}
...
Um die Profile zu finden, stellt man sich die Frage, wie und woran man messen will.\\ 
Soll z.~B. ein Lasttest durchgeführt werden oder der Service in Ruhe sein? Werden die Messungen über einen kurzen oder langen Zeitraum durchgeführt? An welchem Punkt des Services soll gemessen werden?\\
Mit diesen Fragen können die Profile definiert und für die Evaluation festgehalten werden.
Die Tabelle \ref{Anforderungsprofile} zeigt hierzu ein paar mögliche Beispiele. 
...
Falls bereits Expertenwissen vorhanden ist, kann entschieden werden, ob die Evaluation damit erfolgen soll oder nicht. So könnte auch ein Experte die Szenarien durchführen, was diese Phase entsprechend verkürzt, da die Einarbeitungszeit entfällt. Dies könnte aber auch vorbelastete Ergebnisse liefern. Aus diesem Grund kann auch entschieden werden, dass eine Person ohne entsprechende Erfahrungen die Szenarien durchführt.
