...
Ein Beispiel für eine Aufgabe wäre die Verwaltung von Mitarbeitern. Der Service müsste somit eine Datenbankanbindung haben und diese über eine Schnittstelle anbieten. Im Detail können verschiedenste funktionale und nicht-funktionale Anforderungen an den Service gestellt werden, um die Aufgabe zu erfüllen.\\ 
Für das Framework sind nur die Serviceanforderungen interessant, die nicht mit der Geschäftslogik zusammenhängen. Es soll schließlich nur den Rahmen dazu anbieten und den Entwickler weitestgehend bei der Umsetzung der Aufgabe helfen.\\
Welche Anforderungen an einen Service gestellt werden, hängt von der Architektur und dessen Anforderungen ab. Eine besondere Rolle spielen dabei die nicht-funktionalen Anforderungen des Gesamtsystems, da diese teilweise direkt auf die Serviceanforderungen übergehen und für das Framework interessant sind. Beispiele hierfür sind Performance und Sicherheit.
Dabei müssen nicht alle Serviceanforderungen unbedingt direkt aus Anforderungen des Gesamtsystems entstehen. Einzelne Architekturentscheidungen können auch zu Serviceanforderungen führen. Beispielhaft sind hier Service-Discovery und Logging zu nennen. 
...
Diesen Ansatz verfolgt auch die \acf{SAAM}. Das \ac{SEI}, eine US-Bundeseinrichtung für die Verbesserung von Software-Engineering Praktiken, hat mit dieser Methode den Grundstein der Architekturbewertung gelegt.
...
Zuletzt werden den Szenarien Prioritäten zugeordnet, so dass der Fokus zuerst auf kritischen Anforderungen liegt. Der Bewertungs-Workshop ist meist zeitlich begrenzt und somit wird sichergestellt, dass die Nichterfüllung essentieller Qualitätsanforderungen schnell zu einem Abbruch führt. Es kann durch eine einfache Skala mit wenigen Prioritäten erreicht werden, z.~B. A = hoch, B = mittel und C = weniger wichtig.
...
\subsubsection{Resümee \acs*{ATAM}}
...
Zusammenfassend kann gesagt werden, dass \ac{ATAM} eine sehr gute Bewertungsmethode für Architekturen ist und der Einsatz zur Sicherstellung der Qualität empfohlen werden kann.   

\subsection{\acf*{SAEM}}
Im Gegensatz zu \ac{ATAM} verfolgt die \ac{SAEM} Methode anderen Ansatz. Diese betrachtet die Softwarearchitektur als finales Produkt der Entwurfsphase und versucht mittels dem ISO/IEC 25010\footnote{
...
\subsubsection{Resümee \acs*{SAEM}}
Der Unterschied zu \ac{ATAM} ist sehr deutlich. \ac{SAEM} versucht nicht ein tieferes Verständnis für die Architektur bei maßgeblichen Stakeholdern zu schaffen, sondern nur die Einhaltung spezifischer Anforderungen zu gewährleisten. Dies hängt dabei stark vom definiertem Qualitätsziel ab. Gerade bei der internen Qualität verlangt \ac{SAEM} somit eine firmeninterne Qualitätsrichtlinie\cite{IEEE_TSE2002}, die auf Erfahrung und Expertenwissen basiert.
Zusammenfassend kann gesagt werden, dass die Anwendung von \ac{SAEM} schwierig ist und sich nicht alle Ziele der Architekturbewertung aus Abbildung \ref{Architekturbewertung} so deutlich, wie bei \ac{ATAM}, ergeben.

\subsection{Architekturbewertungsmethoden und Frameworks}
Mit Architekturbewertungsmethoden wie \ac{ATAM} oder \ac{SAEM} lässt sich die Qualität einer Architektur gut bewerten. So kann die spezifische Umsetzung einer Microservice Architektur auf die gegebenen Anforderungen untersucht werden. Zudem schärft es nicht nur die Anforderungen selbst, sondern kann auch die geplante Architektur weiter verfeinern und ein tieferes Verständnis bei den Stakeholdern dafür schaffen.

Anhand einer geprüften Microservice Architektur können die passenden Frameworks für die einzelnen Services gewählt werden. 
Hierzu lässt sich die reine Architekturbewertung jedoch nicht mehr verwenden, da diese ein vollständiges Verständnis über die Architektur und dessen Einfluss benötigt. D.~h., in Bezug auf ein Framework muss bekannt sein, wie sich die vorhandenen Architekturentscheidungen und Entwurfsmuster innerhalb des Frameworks auf die tatsächliche Architektur der Umsetzung eines Microservices auswirken.\\
